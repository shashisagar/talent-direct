  ...
  class Envato_Backup {
    ...
    public function check_archive() {
      ...
      if ( ! file_exists( $this->archive_filepath() ) )
        $this->error( $this->archive_method(), esc_html__( 'The backup file was not created', 'jobcareer' ) );
    ...
    public function files() {
      ...
      if ( ! empty( $this->unreadable_files ) )
        $this->warning( $this->archive_method(), esc_html__( 'The following files are unreadable and could not be backed up: ', 'jobcareer' ) . implode( ', ', $this->unreadable_files ) );
