...
class WP_Job_Hunt_Employer_UI {
    ...
    public function init() {
        // Add hook for dashboard employer top menu links.
        add_action('jobhunt_top_menu_employer_dashboard', array($this, 'top_menu_employer_dashboard_callback'), 10, 3);

        // Add Employer left menu and tabs.
        add_action('jobhunt_employer_dashboard_menu_left', array($this, 'add_employer_dashboard_menu_left'), 10, 2);
        add_action('jobhunt_employer_dashboard_tabs', array($this, 'add_employer_dashboard_tab'), 5, 2);

        // Handle AJAX to list all employer job alerts in frontend dashboard.
        add_action('wp_ajax_jobhunt_employer_jobalerts', array($this, 'list_employer_jobalerts_callback'));
        add_action('wp_ajax_nopriv_jobhunt_employer_jobalerts', array($this, 'list_employer_jobalerts_callback'));
    }

    public function top_menu_employer_dashboard_callback($cs_page_id, $uid, $data_toogle) {
        $remove_candidate_role = 'no';
        $remove_candidate_role = apply_filters('jobhunt_remove_candidate_role_frontend', $remove_candidate_role);
        if ($remove_candidate_role != 'yes') {
            echo '<li> <a href="' . esc_url(cs_users_profile_link($cs_page_id, 'job-alerts', $uid)) . '" ' . force_balance_tags($data_toogle) . '><i class="icon-bell"></i> ' . esc_html__('Job Alerts', 'jobhunt-notifications') . '</a> </li>';
        }
    }

    public function add_employer_dashboard_menu_left($profile_tab, $uid) {
        $is_active = '';
        $remove_candidate_role = 'no';
        $remove_candidate_role = apply_filters('jobhunt_remove_candidate_role_frontend', $remove_candidate_role);
        if ($remove_candidate_role != 'yes') {
            ...
            echo ' <li id="employer_left_job_alerts_link" class="' . $is_active . '">
					<a id="employer_postjobs_click_link_id" href="javascript:void(0);" onclick="cs_dashboard_tab_load(\'job-alerts\', \'employer\', \'' . esc_js(admin_url('admin-ajax.php')) . '\', \'' . absint($uid) . '\');" >
    ...
    public function add_employer_dashboard_tab($profile_tab, $uid) {
        $is_active = '';
        $script = '';
        if (isset($profile_tab) && $profile_tab == 'job-alerts') {
            $is_active = 'active';
            $script = ' <script type="text/javascript">
					jQuery(window).load(function () {
						(function (admin_url, cs_uid) {
							var dataString = \'cs_uid=\' + cs_uid + \'&action=jobhunt_employer_jobalerts\';
        ...
        echo ' <div class="tab-pane ' . $is_active . ' fade1 tabs-container" id="job-alerts">
				<div class="cs-loader"></div>
				' . $script . '
			</div> ';
    }

    public function list_employer_jobalerts_callback() {
            ...
            <section class="cs-favorite-jobs">
                ...
                <div class="field-holder">
                    ...
                    if (!empty($cs_plugin_options) && $cs_plugin_options['cs_search_result_page']) {
                        ...
                        ?>
                        <div class="error"><?php esc_html_e('Please select default search list page in Job Plugin Options', 'jobhunt-notifications') ?></div>';
                        <?php
                    }
                    ?>
            ...
            <?php if (!empty($job_alerts) && $job_alerts->have_posts()) { ?>
                        ...
                        while ($job_alerts->have_posts()) :
                            ...
                            /*
                              ...
                              $all_words_search = implode('', array_values($all_words_search));
                              if ( empty($all_words_search) ) {
                              $all_words_search = wp_rem_plugin_text_srt('wp_rem_alerts_all_properties');
                              }
                             */
                            ?>
                ...
                ?>
                            ...
                            if ($alerts_count > $cs_blog_num_post && $cs_blog_num_post > 0) {
                                echo '<nav>';
                                echo cs_ajax_pagination($alerts_count, $cs_blog_num_post, 'job-alerts', 'employer', $uid, '');
                                echo '</nav>';
                            }//==Pagination End 
                            ?>
                        <?php
