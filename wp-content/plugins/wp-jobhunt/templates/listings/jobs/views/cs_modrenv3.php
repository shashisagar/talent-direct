...
<div class="hiring-holder <?php echo cs_allow_special_char($main_col); ?>">
    ...
    ?>
    <ul class="jobs-listing modern-v3">
	<?php
	...
	if ($loop->have_posts()) {
		...
		if ($all_job_type != '') {
			...
			$job_type_values .= '<a ' . force_balance_tags($cs_link) . ' class="apply-btn">' . $job_type->name . '</a>';
		...
		$applications = 0;
		$cs_applicants = count_usermeta('cs-user-jobs-applied-list', serialize(strval($cs_job_id)), 'LIKE', true);
		...
		<li>
			...
			<div class="cs-text">
				...
				?>
				<?php if ($job_type->name <> '') { ?> <li><i class="icon-clock3"></i><?php echo esc_html_e($job_type->name, 'jobhunt');?></li><?php } ?>
				 <li><i class="icon-portfolio"></i><?php echo esc_html__('Applications', 'jobhunt') . ' <span>' . esc_html($applications) . '</span>'; ?></li>
				<?php if ($cs_jobs_address <> '') { ?> <li><i class="icon-map-marker"></i> <?php echo $cs_jobs_address; ?> </li><?php } ?>
				<?php
