...
if (!function_exists('cs_google_login_action')) {
    ...
    function cs_google_login_action() {
        ...
        if (isset($_GET['code'])) {
            ...
            set_transient('google_at', $access_token, 3600);
            header('Location: ' . filter_var(cs_google_login_url(), FILTER_SANITIZE_URL));
            exit;
        }

        $access_token = get_transient('google_at');
        if ($access_token !== false) {
            $client->setAccessToken($access_token);
        }
        if (isset($_REQUEST['logout'])) {
            delete_transient(cs_google_uniqid() . '_google_at');
            $client->revokeToken();
        }
        if ($client->getAccessToken()) {
            ...
            try {
                ...
                set_transient(cs_google_uniqid() . '_google_at', $client->getAccessToken(), 3600);
            ...
            if (isset($_GET['redirect'])) {
                set_transient(cs_google_uniqid() . '_google_r', $_GET['redirect'], 3600);
            }

            $redirect = get_transient(cs_google_uniqid() . '_google_r');

            if ($redirect || $redirect == cs_google_login_url()) {
                $redirect = esc_url(home_url('/'));
                set_transient(cs_google_uniqid() . '_google_r', $redirect, 3600);
            }
...
if (!function_exists('cs_google_redirect')) {
    ...
    function cs_google_redirect() {
        ...
        delete_transient(cs_google_uniqid() . '_google_r');
        exit;
    }
...
if (!function_exists('cs_google_admin_notice')) {
    ...
    function cs_google_admin_notice() {
        $user_info = wp_get_current_user();
        $notice = get_transient($user_info->ID . '_cs_google_admin_notice');
        if ($notice !== false) {
            echo '<div class="updated">
		   <p>' . $notice . '</p>
		</div>';
            delete_transient($user_info->ID . '_cs_google_admin_notice');
        }
    }
