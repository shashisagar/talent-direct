...
/**
 * File Type: Candidate Approved Email Template
 */
if ( ! class_exists('jobhunt_approved_candidate_profile_template') ) {

    class jobhunt_approved_candidate_profile_template {
        ...
        function get_approved_candidate_user_name() {
            $candidate_info = get_user_by('id', $this->candidate_id);
            return $candidate_info->display_name;
        }

        function get_approved_candidate_user_email() {
            $candidate_info = get_user_by('id', $this->candidate_id);
        ...
        public function candidate_profile_status_changed($candidate_id, $candidate_old_status) {
            ...
            if ( $candidate_id != '' ) {
                ...
                if ( $role == 'cs_candidate' ) {
                    ...
                    // checking job status
                    if ( $candidate_status == 'active' && $candidate_status != $candidate_old_status ) {
                        ...
                        if ( isset($template['email_notification']) && $template['email_notification'] == 1 ) {
                            ...
                            $subject = (isset($template['subject']) && $template['subject'] != '' ) ? $template['subject'] : __('Approved Candidate Profile', 'jobhunt');
                            $from = (isset($template['from']) && $template['from'] != '') ? $template['from'] : esc_attr($blogname) . ' <' . $admin_email . '>';
                            $recipients = (isset($template['recipients']) && $template['recipients'] != '') ? $template['recipients'] : $this->get_approved_candidate_user_email();
        ...
        public function add_email_template() {
            ...
            $email_templates[$this->template_group][$this->email_template_index] = array(
                ...
                'description' => __('This template is used to sending email when candidate Status approved', 'jobhunt'),
                'jh_email_type' => 'html',
            );
    ...
    new jobhunt_approved_candidate_profile_template();
}