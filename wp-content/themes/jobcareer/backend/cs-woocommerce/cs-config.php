...
if ( !function_exists( 'jobcareer_loop_add_to_cart' ) ) {
	...
	function jobcareer_loop_add_to_cart() {
		...
		echo apply_filters( 'woocommerce_loop_add_to_cart_link', sprintf( '<a href="%s" rel="nofollow" data-product_id="%s" data-product_sku="%s" data-quantity="%s" class="cs-color btn btn-flat button csborder-color add-to-cart-button ajax_add_to_cart %s product_type_%s">%s</a>', esc_url( $product->add_to_cart_url() ), esc_attr( $product->get_id() ), esc_attr( $product->get_sku() ), esc_attr( isset( $quantity ) ? $quantity : 1  ), $product->is_purchasable() && $product->is_in_stock() ? 'add_to_cart_button' : '', esc_attr( $product->get_type() ), esc_html__( 'Add to cart', 'jobcareer' ) ), $product );
		echo '</div>'; 
	}
...
if ( !function_exists( 'jobcareer_single_product_stock_status' ) ) {
	...
	function jobcareer_single_product_stock_status() {
		...
		if ( $jobcareer_prod_sale == 'instock' ) {
			echo '<span class="stock_wrapper">' . esc_html__( 'Availability', 'jobcareer' ) . ': <span class="stock"><b>' . esc_html__( 'in stock', 'jobcareer' ) . '</b></span></span>';
		} else {
			echo '<span class="stock_wrapper">' . esc_html__( 'Availability', 'jobcareer' ) . ': <span class="stock"><b>' . esc_html__( 'out of stock', 'jobcareer' ) . '</b></span></span>';
		}
	}
