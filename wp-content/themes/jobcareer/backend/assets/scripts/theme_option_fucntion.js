...
jQuery(document).on('click', '#purchase-code-verify-btn', function () {
    var admin_url = jobcareer_theme_options_vars.ajax_url;
    if (jQuery("input[name='item-purchase-code']").val() != '') {
        jQuery('#verify-purchase-code-loader').html(jobcareer_theme_options_vars.purchase_verifiying);
        jQuery.ajax({
            "method": "post",
            "url": admin_url,
            "data": {
            ...
            if (data.success == true) {
                ...
                jQuery('#verify-purchase-code-loader').html(jobcareer_theme_options_vars.verify_code_incorrect);
            }
        }).fail(function () {
            jQuery('#verify-purchase-code-loader').html('');
        });
    } else {
        jQuery('#verify-purchase-code-loader').html(jobcareer_theme_options_vars.verify_blank_error);
    }
});
...
function jobcareer_add_banner_fields(admin_url, theme_url, id) {
    ...
    if (banner_field != "" && check_field_value != "") {
        jQuery("#banner-loader").html("<img src='" + theme_url + "/backend/assets/images/ajax_loading.gif' alt='img_src'/>");
