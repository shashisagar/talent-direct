...
if (!function_exists('jobcareer_clients_shortcode')) {
    ...
    function jobcareer_clients_shortcode($atts, $content = "") {
        ...
        $cs_clients_pagination_switch_str = 'false';
        $slider_fancy_class = '';
        if ($cs_clients_pagination_switch == 'yes') {
            $cs_clients_pagination_switch_str = 'true';
            $slider_fancy_class = ' fancy cs-clients';
        }
        $column_class = '';
        if($column_size != ''){
        ...
        if ($cs_client_style == 'simple' || $cs_client_style == 'modern' || $cs_client_style == 'fancy' || $cs_client_style == 'aviation') {
            if($cs_client_style == 'aviation'){
            $html .= '<div class="cs-clinets aviation-clinets">';
            }else{
         ...
         //   return $html;
            ...
            $html .= '<div class="cs-clinets  '.$slider_fancy_class.' ' . $classes . '">';
            $html .= '<div class="row">';
            ...
            jQuery(document).ready(function () {
                jQuery("#client-slider' . $randomid . '").slick({
                    ...
                    dots:'.$cs_clients_pagination_switch_str.',
                    autoplaySpeed: 2000,
                    responsive: [{
                        breakpoint: 1024,
                        settings: {
...
if (!function_exists('cs_clients_item_shortcode')) {
    ...
    function cs_clients_item_shortcode($atts, $content = "") {
        ...
        if ($cs_client_style == 'simple' || $cs_client_style == 'modern' || $cs_client_style == 'fancy' || $cs_client_style == 'aviation') {
            ...
            if ($cs_client_style == 'aviation') {
                $html .= '<div class="col-lg-4 col-md-4 col-sm-6 col-xs-12"><a href="' . esc_url($cs_website_url) . '"><img src="' . esc_url($cs_client_logo) . '" alt="' . jobcareer_get_post_img_title($post->ID) . '"/></a></div>';
            }
            else {
