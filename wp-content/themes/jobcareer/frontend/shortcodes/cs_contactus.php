...
if (!function_exists('jobcareer_contactform_shortcode')) {
    ...
    function jobcareer_contactform_shortcode($atts, $content = "") {
        ...
        global $cs_plugin_options, $post, $jobcareer_form_fields;
        $view_class = '';
        $defaults = array(
            'column_size' => '',
        ...
        $cs_opt_array = array(
            ...
            'cust_type' => 'email',
            'extra_atr' => ' required  placeholder=\'' . esc_html__('Email Address', 'jobcareer') . '\'',
            'return' => true,
        );
        $html .=$jobcareer_form_fields->cs_form_text_render($cs_opt_array);
        ...
        if ( $cs_terms_condition != '' ) {
            $html .= '<div class="term-conditions">';
                $html .= '<div class="terms">';
                    $html .= '<label><input type="checkbox" name="cs_contact_terms" id="cs_contact_terms" value="on" required="required">';
                        $html .= esc_html__('You accepts our', 'jobcareer');
                        $html .= '<a target="_blank" href="'. esc_url(get_permalink($cs_terms_condition)) .'"> '. esc_html__('Terms and Conditions', 'jobcareer') .'</a>';
                    $html .= '</label>';
                $html .= '</div>';
            $html .= '</div>';
        }
