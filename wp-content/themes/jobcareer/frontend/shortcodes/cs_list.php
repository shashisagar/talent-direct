...
if (!function_exists('cs_list_item_shortcode')) {
    ...
    function cs_list_item_shortcode($atts, $content = "") {
        ...
        if (isset($cs_list_type) && $cs_list_type == 'icon') {
            ...
            $html .= '<li class="' . $border . $icon_bg_class . '" style="list-style-type:none !important;"><i' . $icon_style . ' class="' . $cs_list_icon . '"></i>' . do_shortcode(wp_specialchars_decode($content)) . '</li>';
        } else
        if (isset($cs_list_type) && $cs_list_type == 'default') {
            $html .= '<li class="' . $border . '" style="list-style-type:none !important;">' . do_shortcode(wp_specialchars_decode($content)) . '</li>';
        } else
        if (isset($cs_list_type) && $cs_list_type == 'built') {
            $html .= '<li class="' . $border . '" style="list-style-type:disc !important;">' . do_shortcode(wp_specialchars_decode($content)) . '</li>';
        } else
        if (isset($cs_list_type) && $cs_list_type == 'numeric-icon') {
            $html .= '<li class="' . $border . '" > ' . do_shortcode(wp_specialchars_decode($content)) . '</li>';
        } else
        if (isset($cs_list_type) && $cs_list_type == 'alphabetic') {
            $html .= '<li class="' . $border . '" style="list-style:lower-alpha !important;"> ' . do_shortcode(wp_specialchars_decode($content)) . '</li>';
        }
